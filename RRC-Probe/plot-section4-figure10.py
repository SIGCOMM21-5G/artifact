...
df = pd.read_csv('new_data_combined.csv')
df = df[(df['KeepOrDrop'] == 'YEP')]
# df = df[(df['interval'].isin(time_intervals))]
df['RTT'] = (df['RTT1'] + df['RTT2']) / 2

grp = df.groupby(['radio_type', 'carrier', 'interval', 'Network']).agg(
...
# grp.drop_duplicates(subset=['interval'], keep='last', inplace=True)

grp['color'] = grp.apply(lambda x: determine_color_net_type(translate_network_type(str(x['Network']))), axis=1)


### plotting
dfsatm = grp[(grp['carrier'] == 'Verizon') & (grp['radio_type'] == '4G-LTE')]
fig = plt.figure(figsize=(13, 6))
ax0 = fig.add_subplot(221)
ax0.scatter(dfsatm['interval'], dfsatm['RTT_median'], c=dfsatm['color'])

# dfnsatm = grp[(grp['carrier'] == 'TMobile') & (grp['radio_type'] == '5G-NSA-Low-Band')]
# ax1 = fig.add_subplot(222)
# ax1.scatter(dfnsatm['interval'], dfnsatm['RTT_median'], c=dfnsatm['color'])
#
# dfnsavz = grp[(grp['carrier'] == 'Verizon') & (grp['radio_type'] == '5G-NSA-Low-Band')]
# ax2 = fig.add_subplot(223)
# ax2.scatter(dfnsavz['interval'], dfnsavz['RTT_median'], c=dfnsavz['color'])
#
# dfltetm = grp[(grp['carrier'] == 'TMobile') & (grp['radio_type'] == '4G-LTE')]
