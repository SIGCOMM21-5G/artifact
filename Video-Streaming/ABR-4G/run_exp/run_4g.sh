...
for trace_file in ../cooked_traces/driving/*
    do
        ...
        BACK_PID=$!
        /usr/bin/python run_video.py ${server_ip} fastMPC 200 6 ${filename} 5
        kill ${BACK_PID}
        kill $(ps aux | grep _server | awk '{print $2}') > /dev/null
        bash trace_run.sh ${trace_file} > ./bw_truth/bw_BB_${filename} &
        BACK_PID=$!
        /usr/bin/python run_video.py ${server_ip} BB 200 2 ${filename} 1
        kill ${BACK_PID}
        kill $(ps aux | grep _server | awk '{print $2}') > /dev/null
        bash trace_run.sh ${trace_file} > ./bw_truth/bw_RB_${filename} &
        BACK_PID=$!
        /usr/bin/python run_video.py ${server_ip} RB 200 3 ${filename} 4
        kill ${BACK_PID}
        kill $(ps aux | grep _server | awk '{print $2}') > /dev/null
        bash trace_run.sh ${trace_file} > ./bw_truth/bw_BOLA_${filename} &
        BACK_PID=$!
        /usr/bin/python run_video.py ${server_ip} BOLA 200 4 ${filename} 3
        kill ${BACK_PID}
        kill $(ps aux | grep _server | awk '{print $2}') > /dev/null
        bash trace_run.sh ${trace_file} > ./bw_truth/bw_FESTIVE_${filename} &
        BACK_PID=$!
        /usr/bin/python run_video.py ${server_ip} FESTIVE 200 5 ${filename} 2
        kill ${BACK_PID}
        kill $(ps aux | grep _server | awk '{print $2}') > /dev/null
        bash trace_run.sh ${trace_file} > ./bw_truth/bw_RL_${filename} &
        BACK_PID=$!
        /usr/bin/python run_video.py ${server_ip} RL 200 9 ${filename} 7
        kill ${BACK_PID}
        kill $(ps aux | grep _server | awk '{print $2}') > /dev/null
    done

mv results/* results_driving/