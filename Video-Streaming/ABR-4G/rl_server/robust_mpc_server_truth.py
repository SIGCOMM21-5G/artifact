...
with open('../rl_server/traces/'+'report_' + trace_file[9:]+'oneline', 'r') as truth_trace:
    truth_trace.readline()
    truth_bw = truth_trace.readline().split(',')[0:-1]
    truth_bw = [float(i) for i in truth_bw]
startup_time = time.time()
with open('../run_exp/bw_truth/bw_truthrobustMPC_'+trace_file, 'r') as start:
    ...
    class Request_Handler(BaseHTTPRequestHandler):
        ...
        def do_POST(self):
            ...
            if ( 'pastThroughput' in post_data ):
                ...
                future_bandwidth = future_bandwidth_truth/(1+max_error)
                ...
                for full_combo in CHUNK_COMBO_OPTIONS:
                    ...
                    # linear reward 
                    reward = (bitrate_sum/1000.) - (32*curr_rebuffer_time) - (smoothness_diffs/1000.)
...
def main():
    if len(sys.argv) == 2:
        trace_file = sys.argv[1]
        run(log_file_path=LOG_FILE + '_truthrobustMPC_' + trace_file, bw_file_path=LOG_BW + '_truthrobustMPC_' + trace_file)
    else:
        run()
