...
BITRATE_REWARD_MAP = {0: 0, 2000: 1, 4000: 2, 8000: 3, 12000: 12, 16000: 15, 20000: 20}
M_IN_K = 1000.0
...
# trace_file = 'report_' + trace_file[9:]
trace_start_time = 0
with open('../rl_server/traces/'+'report_' + trace_file[9:]+'oneline', 'r') as truth_trace:
    trace_start_time = float(truth_trace.readline())
    ...
    class Request_Handler(BaseHTTPRequestHandler):
        ...
        def do_POST(self):
            ...
            if ( 'pastThroughput' in post_data ):
                ...
                for idx in range(2):
                    try:
                        curr_idx = int(time.time()-startup_time)+idx
                        if curr_idx >= len(self.ground_truth):
                            curr_idx = len(self.ground_truth)-1
                        future_bandwidth_sum += (1/self.ground_truth[curr_idx])
                    except ZeroDivisionError:
