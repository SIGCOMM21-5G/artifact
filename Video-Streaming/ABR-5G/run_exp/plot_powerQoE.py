...
RESULTS_FOLDER = './results_interface_sel/'
RESULTS_FOLDER_PRED = './results_predict/'
NUM_BINS = 100
BITS_IN_BYTE = 8.0
...
RESULTS_FOLDER_CHK = './results_chunk_length/'
...
def main():
    ...
    for l in time_all_pred[SCHEMES_PRED[0]]:
        schemes_check = True
        # for scheme in SCHEMES:
        # 	if l not in time_all[scheme] or len(time_all[scheme][l]) < VIDEO_LEN or scheme != 'rollbackMPC4s':
        # 		schemes_check = False
        # 		break
        if schemes_check:
            ...
            for scheme in SCHEMES_PRED:
                if scheme == SIM_DP:
                    ...
                    print("raw reward")
                    print(raw_reward_all_pred[scheme][l][1:VIDEO_LEN])
    ...
    # pred.bar(SCHEMES_NEW_PRED, np.array(reward_values)/np.max(np.array(list(mean_rewards.values()))), width, color='r', edgecolor='darkred', ecolor='r', linewidth=2)
    ...
    for idx in range(len(SCHEMES)):
        ...
        print(np.mean(summay_all[SCHEMES[idx]]['all_stalled_rate']))
    ...
    ax0.legend([bar1], ['Bitrate'], fontsize=15, loc='upper center', bbox_to_anchor=(0.85, 1.2), facecolor='whitesmoke') #bbox_to_anchor=(0.15, 1.2),
    ax.legend([bar2], ['Video Stall'], fontsize=15, loc='upper center', bbox_to_anchor=(0.15, 1.2), facecolor='whitesmoke')
