...
# pred_bw = np.load('../rl_server/traces_pred/'+trace_file[:2]+'pred_GDBT'+trace_file[6:]+'.npy')
# print(pred_bw)
with open('../rl_server/traces_pred/'+trace_file[:2]+'pred_GDBT'+trace_file[6:]+'oneline', 'r') as truth_trace:
    pred_bw = truth_trace.readline().split(',')[0:-1]
    pred_bw = [float(i) for i in pred_bw]
startup_time = time.time()
with open('../run_exp/bw_truth/bw_MLpredMPC_'+trace_file, 'r') as start:
            ...
            if ( 'pastThroughput' in post_data ):
                ...
                for idx in range(2):
                    try:
                        ...
                        if current_idx >= len(self.pred_bandwidth):
                            current_idx = len(self.pred_bandwidth)-1
                        if self.pred_bandwidth[current_idx] <= 0:
                            bw = 0.1
                        else:
                            bw = self.pred_bandwidth[current_idx]
                        print(bw)
                        future_bandwidth_sum += (1/float(bw))
                    except ZeroDivisionError:
                        future_bandwidth_sum += 1000   
                    # print(future_bandwidth_sum)
                # print(future_bandwidth_sum)
...
def main():
    ...
    if len(sys.argv) == 2:
        trace_file = sys.argv[1]
        run(log_file_path=LOG_FILE + '_MPC_GDBT_' + trace_file, bw_file_path=LOG_BW + '_MPC_GDBT_' + trace_file)
    else:
        run()
