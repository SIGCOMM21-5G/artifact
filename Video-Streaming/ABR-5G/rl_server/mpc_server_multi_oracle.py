    ...
    class Request_Handler(BaseHTTPRequestHandler):
        ...
        def do_POST(self):
            ...
            if ( 'pastThroughput' in post_data ):
                ...
                self.bw_file.write(str(time.time()) + '\t' + str(future_bandwidth) + '\t' + last_mode + '\n')
                self.bw_file.flush()
                ...
                # Decide if we want to switch to use 4G
                if future_bandwidth < self.threshold and start_buffer < 10:
                    # 5G throughput are low
                    if last_mode == '5G':
                        multipath_req('enable')
                        last_mode = '4G'
                    self.bw_file.write(last_mode + '\n')
                    self.bw_file.flush()
                else:
                    if last_mode == '4G' and rebuffer_time/M_IN_K > 0.5:
                        multipath_req('disable')
                        last_mode = '5G'
                    self.bw_file.write(last_mode + '\n')
                    self.bw_file.flush()
                #start = time.time()
                ...
                # self.bw_file.write(str(total_download_time) + '\n')
...
def main():
    ...
    if len(sys.argv) == 2:
        ...
        run(log_file_path=LOG_FILE + '_intfMPC_' + trace_file + '_no_overhead', bw_file_path=LOG_BW + '_interfaceMPC_' + trace_file)
    else:
        run()
