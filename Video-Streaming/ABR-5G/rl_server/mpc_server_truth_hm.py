...
with open('../rl_server/traces_pred/'+trace_file[:2]+'pred_hm'+trace_file[6:]+'oneline', 'r') as truth_trace:
...
def main():
    ...
    if len(sys.argv) == 2:
        trace_file = sys.argv[1]
        run(log_file_path=LOG_FILE + '_hmMPC_' + trace_file, bw_file_path=LOG_BW + '_truthMPC_' + trace_file)
    else:
        run()
