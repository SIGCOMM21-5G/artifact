...
with open('../run_exp/bw_truth/bw_truthMPC_'+trace_file, 'r') as start:
    startup_time = float(start.readline())
...
def make_request_handler(input_dict):
    ...
    class Request_Handler(BaseHTTPRequestHandler):
        ...
        def calculate_download_time(self, curr_time, chunk_size):
            curr_idx = int(curr_time - startup_time)
            if curr_idx >= len(self.ground_truth):
                curr_idx = len(self.ground_truth)-1
            time_to_next_sec = 1 + startup_time + curr_idx - curr_time # in second
            remaining_size = chunk_size
            down_time = 0.0
            while remaining_size > 0:
                bytes_download = time_to_next_sec * self.ground_truth[curr_idx] * 1000000 # bytes downloads till next chunk s*MBps*1000000 = Bytes
                if bytes_download >= remaining_size:
                    # the chunk download can be finished within this sec
                    down_time += remaining_size / (self.ground_truth[curr_idx] * 1000000)
                    break
                else:
                    down_time += time_to_next_sec
                    time_to_next_sec = 1.0
                    remaining_size -= bytes_download
                    curr_idx += 1
                    if (curr_idx >= len(self.ground_truth)):
                        curr_idx = len(self.ground_truth)-1
            return down_time + 0.15
        ...
        def do_POST(self):
            ...
            if ( 'pastThroughput' in post_data ):
                ...
                for idx in range(3):
                    try:
                        current_idx = int(time.time() - startup_time) + idx
                        if current_idx >= len(self.ground_truth):
                            current_idx = len(self.ground_truth)-1
                        future_bandwidth_sum += (1/self.ground_truth[current_idx])
                    except ZeroDivisionError:
                        future_bandwidth_sum += 0.1    
                future_bandwidth_truth = 1.0/(future_bandwidth_sum/3)
                # future_bandwidth_truth = (self.ground_truth[int(time.time()-startup_time)] + self.ground_truth[int(time.time()-startup_time)+1])/2 
                ...
                for full_combo in CHUNK_COMBO_OPTIONS:
                    ...
                    for position in range(0, len(combo)):
                        ...
                        # download_time = self.calculate_download_time(time.time(), get_chunk_size(chunk_quality, index))
                        # self.bw_file.write(str(download_time) + '\n')
                        # self.bw_file.flush()
                ...
                self.bw_file.write("TOOK: " + str(end-start) + ' download time: ' + str(self.calculate_download_time(time.time(), get_chunk_size(best_combo[0], last_index+1))) + '\n')
